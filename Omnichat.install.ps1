...
if (-not $IsWindows) { throw 'Omnichat installer must be run on Windows PowerShell.' }
$installRoot = 'C:/Omnichat/app'
$desktop = [Environment]::GetFolderPath('Desktop')
Write-Host 'Installing Omnichat to' $installRoot
if (Test-Path $installRoot) { Write-Host 'Clearing existing installation...'; Remove-Item -Path $installRoot -Recurse -Force }
New-Item -ItemType Directory -Path $installRoot -Force | Out-Null
$electronVersion = '28.2.0'
$electronZip = Join-Path $env:TEMP "electron-$electronVersion.zip"
...
Write-Host 'Downloading Electron' $electronVersion 'runtime...'
...
$files = @()
$files += [pscustomobject]@{ Path = 'package.json'; Content = @'
{
  "name": "omnichat",
  "version": "0.1.0",
...
'@ }
$files += [pscustomobject]@{ Path = 'src/main/main.js'; Content = @'
...
'@ }
$files += [pscustomobject]@{ Path = 'src/main/manager.js'; Content = @'
const path = require('path');
...
'@ }
$files += [pscustomobject]@{ Path = 'src/main/utils.js'; Content = @'
...
'@ }
$files += [pscustomobject]@{ Path = 'src/main/log-store.js'; Content = @'
...
'@ }
$files += [pscustomobject]@{ Path = 'src/main/settings-store.js'; Content = @'
const fs = require('fs');
const path = require('path');
const { app } = require('electron');
...
'@ }
$files += [pscustomobject]@{ Path = 'src/preload/index.js'; Content = @'
...
'@ }
$files += [pscustomobject]@{ Path = 'src/preload/agent-preload.js'; Content = @'
...
'@ }
$files += [pscustomobject]@{ Path = 'src/renderer/index.html'; Content = @'
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
'@ }
$files += [pscustomobject]@{ Path = 'src/renderer/styles.css'; Content = @'
:root {
...
'@ }
$files += [pscustomobject]@{ Path = 'src/renderer/renderer.js'; Content = @'
...
'@ }
$files += [pscustomobject]@{ Path = 'resources/selectors.json'; Content = @'
{
  "chatgpt": {
    "input": [
      "textarea",
...
'@ }
$files += [pscustomobject]@{ Path = 'resources/FIRST_RUN.txt'; Content = @'
Welcome to Omnichat!

1. Install Omnichat using the provided installer script.
2. Open Omnichat from your desktop shortcut.
